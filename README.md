...
后续评估的输入覆盖。v2 快照复用每页已有的指标请求，保存基准精确 10 日和半年
收益及逐页观测，不增加外部 API 调用；只有样本门禁通过且该 cohort 的精确基准
输入覆盖率为 100%，才标记市场环境输入就绪。旧版 v1 快照不会回填，任一周期
缺失也会明确保留覆盖缺口。覆盖达标不等于交易日与后验标签就绪或收益验证通过。
